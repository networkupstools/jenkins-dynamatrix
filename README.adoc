...
The library includes a `dynamatrixGlobalState` class with numerous Boolean
toggles which can be enabled to print more debug info in a replayed run,
such as the combos it goes through to create the matrices, or suppressed
stack traces from exceptions. For example, the NUT `Jenkinsfile-dynamatrix`
ends with the following lines to help replay jobs with debugging traces
(beware that it can amount to even gigabytes of log text per run):
...
You can also paste `resources/gist-queue-stats.groovy` into your Jenkins
instance scripting console at `${JENKINS_URL}/script` to help analyze
the Jenkins-Dynamatrix queue pressure during long CI runs (how many items
are waiting for particular build agents due to particular node label
expressions at this moment). This can be used to identify bottlenecks
that can be addressed by either having a more careful specification
of the dynamatrix configuration (which cases are you really interested
in for testing) or adding workers for the heavily stressed platforms.
